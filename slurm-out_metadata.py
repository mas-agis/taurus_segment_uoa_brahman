...
##where I am struggling now --- the idea is if slurm_out has 'BaseRecalibrator' then continue with analysis
##otherwise skip to the next file
    bqsr_Regex=re.compile(r'\bBaseRecalibrator\b')
    bqsr=bqsr_Regex.search(slurm_out).group()                      
    if bqsr=='BaseRecalibrator':
        print("okay ada di " + str(k))
    else:
...
...         print(n, 'is a prime number')
    ...
    if re.compile(r'(BaseRecalibrator -)
##until here 
